...
\alert{SM: I'll rewrite the abstract}

\end{abstract}

\begin{IEEEkeywords}
Deep Learning, Hardware-in-the-Loop, Neural Network, Plant Modeling, System Identification
\end{IEEEkeywords}
...
vehicle platform, but their \comment{reliability}{Need to define what do you mean by reliability, be very specific} is lower than expected due to non-linear
behaviors of the torque voltages required to control each subsystem. The scope
of this project includes developing mathematical \add{and/or data-driven} models for each subsystem so \add{that one} \remove{AutonomouStuff} can \add{easily} implement control systems that improve the reliability of the
autonomous vehicle platform. These models will be considered reliable if they can track actual vehicle subsystem behaviors with a minimum best fit percentage between 85 to 95 percent and fall within the error bounds defined for each subsystem. Once these models are developed, they will be tested using \remove{AutonomouStuff's} Hardware-in-the-Loop (HIL) bench.
Once there is confidence in each model, \remove{AutonomouStuff can use} these models \add{can be used} on \add{the}  \remove{their}  HIL bench to develop controllers that will remove the non-linear behaviors of each vehicle subsystem.
...
\end{itemize}
%
\todo[inline]{NN/HG: Add a sentence on what does manual-drive operating mode mean? You mentioned this in your presentation already. Then you pass on to autonomous mode. }
...
turn the pinion arms which would change the steering angle of the vehicle. \comment{This
principle is applied to all other subsystems.}{Does this mean that this principle will be applicable to acceleration subsystem as well? If not, you will need to modify this statement.}
...
\end{figure}
 ...
 create different models, \remove{they}\add{raw data} collected \remove{and used data} from an energy center in
 Malaysia \add{was used}. After generating different models, the authors ended up going with a
...
The example given in~\cite{example1} shows how to create an \comment{iddata}{What do you mean? Could you please define this term from high-level perspective?} object from a
...
The frequency response of the model was estimated with
spectral analysis and bode \replace{plots}{diagrams} were also created. The tutorial explained that
if the data \replace{doesn't}{does not} give \replace{nice}{acceptable} models, then it is best to try out submodels for
...
this model and comparing it to the validation data and previous models, \replace{we saw}{it is observed} a
best fit percentage of 90.18\%. The last thing the example showed was how to
merge the two SISO models we created earlier. %
%
\todo[inline]{Fig.~\mbox{\ref{fig:exMIMO}} was not referred in the previous paragraph. At least you should say, Fig... shows $\ldots$. A figure cannot be placed without explaining what the figure is about in the section.}
\begin{figure}[htbp]
    \centering
...
\end{figure}
%
\todo[inline]{Same thing happened here. A figure (Fig. 3) is placed without explaining what this figure is about in the text. }
...
\end{figure}
% 
\todo[inline]{Same thing happened here. A figure (Fig. 4) is placed without explaining what this figure is about in the text. }

\section{System Architecture}
The overall system architecture of this project consists of six subsystems which are the steering, acceleration, brake, speed, shift, and speed control systems. \add{Fig.~\ref{fig:highLevelSystemArchitecture} outlines these six subsystems in a Lexus self-driving vehicle provided by company (Hexagon $|$ AutonomouStuff).} %
% 
\begin{figure}[htbp]
  \centering
...
\end{figure}
%
\add{However, actual vehicle (Lexus RX450H) that is used for collecting input-output data for all of these subsystems is shown in the picture of Fig.~\ref{fig:lexusvehicle}. A team of the project members visited the site to collect raw data for steering, brake, and acceleration subsystems of the vehicle.} %
% 
\begin{figure}[htbp]
	\centering
...
\end{figure}
%
\add{The error signal, which is the difference between the target and the actual directions of the vehicle, is passed on to the steering controller. The outputs of the steering controller are the torque voltages that are applied to the power steering motors of the steering subsystems of the vehicle.} 

  \subsection{Brake Subsystem}
  The brake subsystem is made up of the master cylinder, brake rotor, brake drum, brake pad, brake caliper, brake shoe, brake booster, brake pedal, wheel speed sensors, ABS module, and brake lines~\footnote{\url{https://oards.com/car-brake-system-components/}}. \add{A high-level block diagrams showing the inputs and puts of a conventional brake subsystem of a self-driving vehicle is shown in Fig.~\ref{fig:brakeModelArchitecture}.} %
%  
 \begin{figure}[htbp]
    \centering
  \begin{tikzpicture}
...
\todo[inline]{Please see how I described block diagrams for steering, and brake subsystems. Use the same format for explaining all the subsystems.  You cannot just place a figure without referring and explaining the figure in the text.} 
 ...
 \begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \tikzstyle{block} = [draw, rectangle, fill=green!15, rounded corners=5pt, minimum width=1.5 cm, minimum height=1.5 cm]
    % place nodes
    \node[block,text width=1.75cm,text centered](plant){MIMO system};

    % Connections
	\draw[stealth-,thick]
  (plant.150)--++(-1.2,0)node[left](in1){$x_1$};    
    \draw[stealth-,thick]
    (plant.170)--++(-1.2,0)node[left](in2){$x_2$};
    \draw
    ($(plant.180)+(-0.5,0)$)node[yshift=-0.9mm]{$\vdots$};
    \draw[stealth-,thick]
    (plant.-150)--++(-1.2,0)node[left](inn){$x_n$};
    
    \draw[-stealth,thick]
    (plant.30)--++(1.2,0)node[right](out1){$y_1$};
    \draw[-stealth,thick]
    (plant.10)--++(1.2,0)node[right](out2){$y_2$};
    \draw
    ($(plant.0)+(0.5,0)$)node[yshift=-0.9mm]{$\vdots$};
    \draw[-stealth,thick]
    (plant.-30)--++(1.2,0)node[right](outm){$y_m$};
  \end{tikzpicture}
...
\end{figure}
%
\todo[inline]{SM: I'll rewrite the following paragraph since the description herein is valid for specific architecture of an actor neural network. } 
The neural network inputs are mapped to the outputs by being assigned a set of weights~\footnote{\tiny \hsize=0.45\textwidth \url{https://towardsdatascience.com/machine-learning-for-beginners-an-introduction-to-neural-networks-d49f22d238f9}}. 
...
\replace{In order t}{T}o accurately model each vehicle subsystem so that safe, reliable controllers can be developed for autonomous control, the following criteria will be met:
\begin{itemize}
...
\item Each vehicle subsystem plant model will track any desired inputs within the specified error bounds\add{:}
	\begin{itemize}
		\item Steering \replace{A}{a}ngle within 5 degrees
		\item Pedal \replace{P}{p}ositions within 5\%
	\end{itemize}
...
All of the subsystems have nonlinear behaviors when there are small changes in the output of the subsystem. The steering subsystem, for example, should behave in a smooth, continuous manner. However, \add{a team of researchers from }AutonomouStuff observed that when trying to implement features such as lane tracking for the Lexus vehicle platform, that when small changes in the steering angle (less than five degrees) occurred, the torque voltages would momentarily stall and then suddenly change causing a more drastic change. This behavior, depicted in Fig.~\ref{fig:nonlinGraph}, made it very challenging to complete features like lane tracking. \replace{These}{The plant models developed in this work}\remove{ models} will aid in the development of controllers that will remove the nonlinearities allowing features like lane tracking to be implemented in a safer, smoother manner. 
...
\todo[inline]{The text of the following paragraph is very similar to the above section except that the word ``steering'' was replaced by ``acceleration''. You paraphrase the sentences and there should some differences in the input and output data. What are the input data, the output data, units of those data, for example}
...
We \replace{traveled to}{visited the workplace of} AutonomouStuff in order to collect
...
\noindent The \add{interfacing software,} \todo[fancyline]{name of the software}, \add{installed in the} laptop is used to pass commands or log data, such as steering angle or acceleration or brake pedal position. This data is sent or received using the CANCase and CAN bus. These are connected to the AutonomouStuff designed PACMod ECU, which sends torque voltages to the desired vehicle subsystem allowing the laptop to either control the desired vehicle system or log data. The Vector CANAlyzer software is installed on the laptop and is used to parse the collected data that is sent from the CANCase. This is how we were able to collect logs of data that we would use to develop \add{plant} models of the autonomous vehicle subsystems.
...
\vspace{12pt}
\noindent The model of the by-wire steering system is a twentieth order transfer
function. The twentieth order transfer function is the best proposed estimation
considering the system costs associated with a higher-order transfer function
while also obtaining an acceptable best fit percentage. %
%
\begin{table}[hbtp]
  \centering
...
\end{table}
%
Table~\ref{tab:byWireSteerCoeffA} shows the coefficients of the twentieth order transfer function for the output steering angle with respect to the input torque voltage A signal.%
% 
\add{The second set of coefficients for the torque voltage A is given below:} %
%
\begin{center}
...
\end{table}
%
\add{Similar to the torque voltage A, the second set of coefficients for the torque voltage B signal is shown below:} %
%
\begin{center}
  \resizebox{0.4\linewidth}{!}{% Resize table to fit within \linewidth horizontally
    \begin{tabular}{*{21}{c}}
...
\end{table}
%
\add{This is followed by the $b$-coefficients given below:} %
%
\begin{center}
  \resizebox{0.4\linewidth}{!}{% Resize table to fit within \linewidth horizontally
    \begin{tabular}{*{21}{c}}
...
\todo[inline]{Figures of both steering and acceleration subsystem models are placed without referring them in the paragraph. Each of the figures needs to be explained. Also, the following paragraph and the paragraph for the acceleration system are alike except that the word ``steering'' is replaced with the word ``acceleration''. This should not be the case in a professional report.}
...
\begin{figure}
  \centering
    \includegraphics[width=0.95\linewidth]{figs/inkscape/hilBenchAstuffAnnotated}
