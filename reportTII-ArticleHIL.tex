...
\end{figure}
 ...
 create different models, raw data collected from an energy center in
 Malaysia was used. After generating different models, the authors ended up going with a
...
spectral analysis and bode diagrams were also created. The tutorial explained that
if the data does not give acceptable models, then it is best to try out submodels for
...
this model and comparing it to the validation data and previous models, it is observed a
best fit percentage of 90.18\%. The last thing the example showed was how to
merge the two SISO models we created earlier. Fig.~\ref{fig:exMIMO} shows the results of comparing the estimated state space model along with the merged SISO models to the given validation data. 
%
...
The overall system architecture of this project consists of six subsystems which are the steering, acceleration, brake, speed, shift, and speed control systems. Fig.~\ref{fig:highLevelSystemArchitecture} outlines these six subsystems in a Lexus self-driving vehicle provided by company (Hexagon $|$ AutonomouStuff). %
% 
\begin{figure}[htbp]
  \centering
...
\end{figure}
%
However, actual vehicle (Lexus RX450H) that is used for collecting input-output data for all of these subsystems is shown in the picture of Fig.~\ref{fig:lexusvehicle}. A team of the project members visited the site to collect raw data for steering, brake, and acceleration subsystems of the vehicle. %
% 
\begin{figure}[htbp]
	\centering
...
To accurately model each vehicle subsystem so that safe, reliable controllers can be developed for autonomous control, the following criteria will be met:
\begin{itemize}
...
% 
The second set of coefficients for the torque voltage A is given below: %
%
\begin{center}
...
\end{table}
%
Similar to the torque voltage A, the second set of coefficients for the torque voltage B signal is shown below: %
%
\begin{center}
  \resizebox{0.4\linewidth}{!}{% Resize table to fit within \linewidth horizontally
    \begin{tabular}{*{21}{c}}
...
\end{table}
%
This is followed by the $b$-coefficients given below: %
%
\begin{center}
  \resizebox{0.4\linewidth}{!}{% Resize table to fit within \linewidth horizontally
    \begin{tabular}{*{21}{c}}
