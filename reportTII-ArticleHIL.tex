...
dataset in order to get the inputs and the outputs. An iddata object is a way to format input and output time-series data for the manipulation of data that MATLAB employs. The next step was to look at
...
was the most accurate, with a best fit percentage of 85.83\% as depicted in Fig.~\ref{fig:exIO}. The plot shows the effects of the two inputs, consumption rate and current, as well as the simulated output of the production.
%
\begin{figure}[htbp]
    \centering
    % \captionsetup{justification=centering, margin=3cm}
...
%
In Fig.~\ref{initalSteeringModel}, the top plot shows the simulated output of the neural network when the input torque voltages and desired steering angle from data subset 1 are fed into the model. The bottom plot shows the error between the desired steering angle and the output of the neural network. For the steering model, the error should be less than 5 degrees or approximatley 0.1 radians. Fig.~\ref{testedSteeringModel} shows the same plots except data subset 2 was used instead.

\begin{figure}[htbp]
	\centering
	\subcaptionbox{Model Output Tracking and Error Plots for Data Subset 1 \label{initalSteeringModel}}
