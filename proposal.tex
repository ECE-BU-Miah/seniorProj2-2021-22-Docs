...
The overall system architecture of this project consists of six subsystems which are the steering, acceleration, brake, speed, shift, and cruise control systems as shown in \autoref{fig:subFunctionalBlock}. Each of the six subsystems can be described as a multiple input system because there are two torque voltage signals used to describe the behavior of each vehicle subsystem. Some of the systems are multiple output and some are single output as depicted in \autoref{fig:subFunctionalBlock}.
...
\end{figure}
 ...
 \begin{itemize}
    \item The steering subsystem will be modeled first due to its non-linearities, depicted in \autoref{fig:nonlinGraph}, and it is important to the operation of the vehicle
...
\begin{figure}[h]
	...
	\subcaptionbox{Output of Estimated By-Wire System Model \label{byWireSteerModel}}
		{\includegraphics[width=0.4\linewidth]{figs/img/byWireSteeringTransferFunctionModel}}
	\hfill
	\subcaptionbox{Output of Estimated Manual System Model \label{manualSteerModel}}
		{\includegraphics[width=0.4\linewidth]{figs/img/manualSteeringTransferFunctionModel}}
	\caption{Steering System Estimated Steering Angle Comparison}
\end{figure}

\vspace{12pt}
\noindent The model of the by-wire steering system is a twentieth order transfer function. The twentieth order transfer function is the best proposed estimation considering the system costs associated with a higher order transfer function while also obtaining an acceptable best fit percentage. \autoref{byWireSteerCoeffA} shows the coefficients of the twentieth order transfer function for the output steering angle with respect to the input torque voltage A signal. \autoref{byWireSteerCoeffB} shows the coefficients of the twentieth order transfer function for the output steering angle with respect to the input torque voltage B signal.
\begin{figure}[h]
...
\begin{figure}[h]
    ...
    \end{tabular}}
	\end{center}
	\label{byWireSteerCoeffB}
	\caption{By-Wire Mode Steering Transfer Function Torque Voltage B Coefficient Table}
\end{figure}

\vspace{12pt}
\noindent The model of the manual steering system is a twentieth order transfer function. The twentieth order transfer function is the best proposed estimation considering the system costs associated with a higher order transfer function while also obtaining an acceptable best fit percentage. \autoref{manualSteerCoeffA} shows the coefficients of the twentieth order transfer function for the output steering angle with respect to the input torque voltage A signal. \autoref{manualSteerCoeffB} shows the coefficients of the twentieth order transfer function for the output steering angle with respect to the input torque voltage B signal.
...
\begin{figure}[h]
	...
	\subcaptionbox{Output of Estimated By-Wire System Model \label{byWireAccelModel}}
		{\includegraphics[width=0.4\linewidth]{figs/img/byWireAccelArxModel}}
	\hfill
	\subcaptionbox{Output of Estimated Manual System Model \label{manualAccelModel}}
		{\includegraphics[width=0.4\linewidth]{figs/img/manualAccelTransferFunctionModel}}
	\caption{Acceleration System Estimated Pedal Position Comparison}
\end{figure}

\vspace{12pt}
\noindent The model of the by-wire acceleration system is a fourth order ARX model. The fourth order ARX model defined in \autoref{byWireAccelEq} represents the output acceleration pedal position, A(z), based on input torque voltage A, $B_1(z)$, and input torque voltage B, $B_2(z)$. 
\begin{figure}[h]
	\begin{equation}
	...
	\end{equation}
	\label{byWireAccelEq}
	\caption{By-Wire Mode Acceleration System ARX Model Equations}
\end{figure}

\vspace{12pt}
\noindent The model of the manual acceleration system is a twenty-fourth order transfer function. The twenty-fourth order transfer function is the best proposed estimation considering the system costs associated with a higher order transfer function while also obtaining an acceptable best fit percentage. \autoref{manualAccelCoeffA} shows the coefficients of the twenty-fourth order transfer function for the output acceleration pedal position with respect to the input torque voltage A signal. \autoref{manualAccelCoeffB} shows the coefficients of the twenty-fourth order transfer function for the output acceleration pedal position with respect to the input torque voltage B signal.
...
%     \ganttgroup[progress = 80]{Simulation}{8}{10}\\
     ...
     \ganttbar[progress = 50]{Proposal Document}{9}{15}\\
     \ganttbar[progress = 30]{Proposal Presentation}{9}{15}\\

%     \ganttgroup[progress = 82.5]{XBee Setup}{13}{14}\\
     \ganttmilestone[progress = 30]{Model Steering System}{13}{14}\\
     \ganttbar[progress = 0]{Model Acceleration System}{14}{15}\\
     \ganttbar[progress = 0]{Model Braking System}{15}{16}\\
