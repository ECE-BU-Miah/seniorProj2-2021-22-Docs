...
Once there is confidence in each model, AutonomouStuff can use these models on their HIL bench to develop controllers that will remove the non-linear behaviors of each vehicle subsystem. By developing models for these vehicle subsystems, this allows AutonomouStuff to save money and time that would normally be spent on vehicle maintenance. This is the first time that the company will have a model of one of their vehicles. 
There are two operating modes of the Lexus vehicle platform used in this project: Manual-drive and by-wire or autonomous. Originally, models were created for the steering and acceleration subsystems in both manual and by-wire mode. However, as the project progressed, it was decided that only models in manual mode needed to be created. The controller handles most of the self-driving characteristics, and since the subsystem models will be used to develop new controllers, it doesn’t make sense to model the present controllers if they are going to change. 
...
%     \ganttgroup[progress = 0]{Assembly}{3}{4}\\
     \ganttbar[progress = 0]{Collect Shift, Speed Control, and Speed Data}{4}{4}\\
     \ganttbar[progress = 0]{Model Shift, Speed Control, and Speed Subsystems}{4}{5}\\
     \ganttbar[progress = 0]{Progress Presentation}{6}{6}\\
...
%     \ganttgroup[progress = 10]{Software}{5}{6}\\
     \ganttbar[progress = 0]{Test Subsystems}{6}{7}\\
     \ganttmilestone[progress = 0]{Test Subsystems with HIL}{7}{7}\\
     \ganttbar[progress = 0]{AutonomouStuff Demo}{8}{9}\\
...
We will create a presentation that outlines the work that we have completed up to this point as well as what we intend to complete for the remaining time of the project. We will be evaluated on our ability to demonstrate that we are making progress on the project and on our ability to complete the goals that we set.  

\item[Collect shift, speed control, and speed data]: 
Once we have satisfactorily modeled the steering, acceleration, and brake systems, we will then travel to AutonomouStuff and collect the necessary data for the shift, speed control, and speed subsystems. We will make sure to collect a large amount of data to work with. This process should only take a day. 
  
\item[Model shift, speed control, and speed subsystems]: 
Once we have collected the necessary data from the Lexus vehicle, we will work on developing individual mathematical models for each subsystem. We will be using MATLAB’s System Identification Toolbox in order to try out different models and see which best represents each subsystem. These models will first be tested by us, and if they pass they will then be tested using AutonomouStuff’s HIL testbench. 


\item[Test subsystems]: 
Before we do any HIL testing, we will first work on testing the models ourselves in order to ensure that they are accurate. We are still determining the best way to test these models, but this is an important step since we do not want to take inaccurate models to be tested using the HIL testbench. Using an inaccurate model would cause us to lose time that could have been spent coming up with a better one. 

\item[Test subsystems with HIL]: 
Once we have modeled our subsystems, we will then take these models and test them using AutonomouStuff’s HIL testbench. This allows us to see how our models will perform in the real world. Once these models have been tested, they will either be changed or kept as is. 

\item[AutonomouStuff demonstration]: 
Around the time we test the subsystems using HIL, we will also demonstrate what we have worked on to representatives from AutonomouStuff. This is an important demonstration since AutonomouStuff has sponsored this senior project and is waiting to see if we have solved the problem of creating accurate mathematical models for their vehicle subsystems.

\item[Final report]: 
The final report will sum up all of our efforts and findings for this project. It will contain all of the information so that readers will be able to understand how we generated the models and how we collected the necessary data. We will make sure to detail the steps we took and especially how we dealt with the non-linearity considerations for the steering system.

\item[Final presentation]: 
Similarly to the fall semester, we will give a complete presentation of the work we have done on this project and the resulting models we have come up with. This presentation will be conducted in front of a faculty panel, along with AutonomouStuff representatives and other interested parties. It will be the culmination of everything we have done over the course of our senior year. 

\item[Presentation to IAB]: 
Near the end of the semester we will be presenting our work on this project to the Industrial Advisory Board, or IAB. This will allow the board to see some of the projects that the senior class has been working on this year. Since the IAB plays a large role in choosing senior projects, it will be important to show them the progress we have made on one of the projects they selected. 
