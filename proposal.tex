...
% \end{tiny}
 ...
 \end{align*}
% 
$y(t)$ is the system output at time $t,$ while $u(t)$ is the input. \comment{}{Didn't you say, it was a two input, one output system? Where is the other input in the above equation, other than $u(t)$?} The noise disturbance of the system is represented by $e(t).$ The variables $na,$ $nb,$ and $nk$ are the system’s number of poles, amount of $b$ parameters, and the samples before the inputs begin to affect the system’s output.
...
\todo[inline]{Place MATLAB figures for this example here. }
...
\end{figure}
  ...
  The acceleration subsystem is a multiple-input multiple-output (MIMO) system. Acceleration pedal voltages are sent to the subsystem. A new acceleration pedal position value is generated to better match the real-time pedal position. 
 \begin{figure}[h]
    \centering
    \captionsetup{justification=centering, margin=3cm}
