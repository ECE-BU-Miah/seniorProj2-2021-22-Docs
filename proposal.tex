...
behaviors of the torque voltages required to control each subsystem. \hl{The scope
...
%
\\
\noindent Using the data we collected for the steering, acceleration, and braking subsystems, we initially separated the data so we could analyze data for the by-wire mode and manual mode individually. This effort was made to identify if the models we developed could be used interchangeably regardless of what mode the autonomous vehicle subsystems are operating in. After analyzing the models we developed, we determined that these models are not interchangeable as there were differences in the model. When the vehicle subsystem is in by-wire mode, the controller AutonomouStuff developed is generating the torque voltages that are applied to each subsystem motor. Because of this, we decided the most accurate model of each subsystem would be developed using the data when the subsystems are in manual mode. For reference, the differences in the models created for by-wire and manual mode are depicted in \ref{sec:simresults} for the steering and acceleration subsystems. For all other models we develop, we will only use manual mode data and will not create models for by-wire mode.
...
We will create a presentation that outlines the work that we have completed up to this point as well as what we intend to complete for the remaining time of the project. We will be evaluated on our ability to demonstrate that we are making progress on the project and will be able to complete the goals that we set.  

\item[Collect shift, speed control, and speed data] 
Once we have satisfactorily modeled the Steering, Acceleration, and Brake systems, we will then travel to AutonomouStuff and collect the necessary data for the shift, speed control, and speed subsystems. We will make sure to collect a large amount of data in order to ensure that we will get accurate models. This process should only take a day. 
...
Once we have modeled our subsystems, we will then take these models and test them using AutonomouStuff’s Hardware in the Loop testbench. This allows us to see how our models will perform in the real world. Once these models have been tested, they will either be changed or kept as is. 
